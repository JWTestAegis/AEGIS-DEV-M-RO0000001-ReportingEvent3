...
message("[AEGIS Logging]Script Starts!IT011-ruser3")
###########################
...
if (length(args) < 4) {
  ...
  #  quit(status=1)
  message("[AEGIS Logging]Usage: Rscript filter_sas_to_xpt.R <input.sas7bdat> <column> <value> <output_xpt> <run_user>")
...
message(paste0("[AEGIS Logging]Arguments detail... input=",in_path,", column=",col,", searchValue=",val,", ouput=",out_xpt,", run_user=",actual_executor))
###########################################
...
if(!file.exists(out_xpt)) {
  ...
  ### Using Logger: Argument error ###
  #log_error("Output not found", action="output_check", expected="Output must be generated as an actual xpt file", output=out_xpt)
  message("[AEGIS Logging]Output not found, output must be generated as an actual xpt file.")
  ####################################
...
out_hash <- digest::digest(file = out_xpt, algo = "sha256")
#log_info("Output completed", action="output_generate", path=out_xpt, 
#         duration_ms=elapsed1_ms, sha256=out_hash, format="xpt")
message(paste0("[AEGIS Logging]Output completed. output: ", out_xpt, ", Hash: ", out_hash, ", Duration: ", elapsed1_ms))
#################################
